...
#生成された基点にx方向に架かる低いブリッジの生成
    b=gh.CenterBox(gh.XYPlane(xz_pos),b_pos_x[i][1]/2+b_width/2,b_depth/2,b_hight[0]/2)
    bridge_x.append(b)
for i in range(0,len(b_pos_x)):
...
#生成された基点にx方向に架かる高いブリッジの生成
    xz_pos=gh.ConstructPoint(b_pos_x[i][0][0],b_pos_x[i][0][1],b_z)
    b=gh.CenterBox(gh.XYPlane(xz_pos),b_pos_x[i][1]/2+b_width/2,b_depth/2,b_hight[1]/2)
    bridge_x.append(b)
...
#生成された基点にy方向に架かる低いブリッジの生成
    b=gh.CenterBox(gh.XYPlane(yz_pos),b_depth/2,b_pos_y[i][1]/2+b_width/2,b_hight[0]/2)
    bridge_y.append(b)
for i in range(0,len(b_pos_y)):
#b_zにて高いブリッジの断面方向配置位置を生成, yz_posにて低いブリッジを生成するための基点生成
    b_z=Hi[random.randrange(0,len(Hi))]
...
#生成された基点にy方向に架かる高いブリッジの生成
    b=gh.CenterBox(gh.XYPlane(yz_pos),b_depth/2,b_pos_y[i][1]/2+b_width/2,b_hight[1]/2)
    bridge_y.append(b)
